...
import {
  ADMIT_INSTRUCTIONS, admitDensityFor, admitDensityKeyFor,
  admitInstructionLabel, admitLineCount, admitPrintCss,
} from './lib/admitCardLayout'
  ...
  return `
    <article class="admit-card" style="width:190mm;min-height:277mm;margin:0 auto;padding:7mm;background:#fff;color:#021024;border:2px solid #000;overflow:hidden;position:relative;page-break-after:always;page-break-inside:avoid;break-inside:avoid;font-family:Arial,sans-serif;letter-spacing:0;box-sizing:border-box;">
      ...
      <section class="admit-instructions" style="position:relative;z-index:6;margin:0 6mm 5mm;padding:${density.instrPad};background:#f8fbff;border:1px solid #b9d2f0;border-radius:2mm;page-break-inside:avoid;">
        ...
        <ol style="margin:0;padding:0;list-style:none;font-size:${density.instrFont};line-height:${density.instrLine};">
          ${ADMIT_INSTRUCTIONS.map((text, index) => `<li style="display:grid;grid-template-columns:6mm 1fr;gap:1mm;">
            <span>${admitInstructionLabel(index)}</span><span>${escapeHtml(text)}</span>
          </li>`).join('')}
        </ol>
      </section>
      ...
      <div class="admit-footer-note" style="position:absolute;left:13mm;right:13mm;bottom:8mm;z-index:6;display:flex;justify-content:space-between;gap:8mm;border-top:1px solid #cbd5e1;padding-top:2mm;color:#334155;font-size:7.5pt;">
        <span>Generated by SCHOOL99</span>
...
function AdmitCardPaper({ student, exam = {}, dateRows = [], school = {}, settings = {}, showPendingFee = false, pendingAmount = 0 }) {
  ...
  return <article className={`admit-card density-${densityKey}`}>
    ...
    <section className="admit-instructions">
      <h3>Instructions :</h3>
      <ol className="admit-instruction-list">
        {ADMIT_INSTRUCTIONS.map((text, index) => <li key={text}><span>{admitInstructionLabel(index)}</span><span>{text}</span></li>)}
      </ol>
    </section>
